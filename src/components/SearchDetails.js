...
const SearchDetails = (props) => {
  ...
  const [petReviewes, setPetReviews] = useState(false);
  ...
  const openPetsHandler = () => {
    setOpenPets(!openPets);
    setCoupleToggle(false);
    setOpenFilter(false);
  };

  const openCoupleHandler = () => {
  ...
  const nonPetsHandler = () => {
    setOriginalArray(placeDetailsArray);
    setPetReviews(false);
  };

  const coupleFilter = () => {
    // filtering couple friendly options
    let mutate = placeDetailsArray.filter((item) => {
    ...
    <div className="searchDeatilsMainParent">
      {!coupleToggle && !openPets && (
        <p className="text-2xl flex relative uppercase fof left-24 top-36">
          Toatl {placeDetailsArray.length} Stays in {loc}{" "}
        </p>
      )}
      ...
      <div className="functionalityButtonsHold cursor-pointer flex gap-4">
        ...
        <div className="flex coupleHold mt-3 zoom" onClick={openCoupleHandler}>
          <img
            src="https://cdn.iconscout.com/icon/premium/png-256-thumb/couple-3431093-2863604.png"
      ...
      {openFilter && (
        <div className="dropdownFilter rounded-2xl capitalize">
          <p className=" cursor-pointer pt-10 pr-4" onClick={lowToHighFilter}>
            {" "}
            Low to high{" "}
          </p>
          <br />
          ...
          <div className="relative arrowsHold">
            <img
              src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSPh9c_dLqYXwYwXePJTwBAG8nTZG0V2LdAtA&usqp=CAU"
              className="h-4"
            />
            <br />
          </div>
      ...
      {openPets && (
        <div className="selectDropDown">
          ...
          <div onClick={nonPetsHandler} className="cursor-pointer h-2">
            No{" "}
          </div>
        </div>
      )}
      ...
      <div className="zindex">
        {coupleToggle && (
          <div className="selectDropDownCouple cursor-pointer">
            <div
              onClick={coupleFilter}
              className=" pt-4 mb-8 cursor-pointer h-2"
            >
              {" "}
              Yes{" "}
            </div>
            <div onClick={noncoupleHandler} className="cursor-pointer h-2">
              No{" "}
            </div>
          </div>
        )}
